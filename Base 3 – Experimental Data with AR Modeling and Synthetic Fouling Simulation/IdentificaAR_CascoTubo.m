...
% Separate datasets
temperatura_casco_limpo_exp = All_Curves(:, 1:n_exp)';  % Experimental clean curves
temperatura_casco_limpo_sim = All_Curves(:, n_exp+1:n_exp+n_sim)'; % Simulated clean curves
temperatura_casco_sujo = All_Curves(:, (n_exp+n_sim+1):end)';    % Fouled curves

% Compute standard deviation for correct noise scaling
std_clean = std(temperatura_casco_limpo_sim, 0, 2); % Compute std per row
std_fouled = std(temperatura_casco_sujo, 0, 2); 

% Generate noise with the same shape as the data
noise_clean = fator * std_clean .* randn(size(temperatura_casco_limpo_sim));
noise_fouled = fator * std_fouled .* randn(size(temperatura_casco_sujo));

% Add noise to the simulated data
temperatura_casco_limpo_sim = temperatura_casco_limpo_sim + noise_clean;
temperatura_casco_sujo = temperatura_casco_sujo + noise_fouled;

% Combine clean experimental and simulated data
temperatura_casco_limpo = [temperatura_casco_limpo_exp; temperatura_casco_limpo_sim];
...
np_train = np(1);  
y = temperatura_casco_limpo(np_train, :);  % Training data
...
x_v = temperatura_casco_limpo(np_val, :); % Validation data
...
for i = 1:(n_exp+n_sim-1)
   x = temperatura_casco_limpo(:, clean_indices(i)); % Use shuffled indices
...
for i = 1:n_fou
   x = temperatura_casco_sujo(:, fouled_indices(i)); % Use shuffled indices
   % x = x + fator*sqrt(mean(x.^2))*randn(length(t),1);
   %x = zscore(x);
   data_x=iddata(x,[],dt);
   erro = pe(data_x,model_ref);
...
energia_limpo = trapz(t, temperatura_casco_limpo_corrigido', 1); % Integral para curvas limpas corrigidas com ruído
energia_sujo1 = trapz(t, temperatura_casco_sujo', 1);  % Integral para curvas sujas 1 com ruído
...
%-------------------------------------------------------------------
save('features_CascoTubo.mat', 'X_1', 'X_2'); % Save to .mat file
save('model_CascoTubo','model_ref','coeficiente_limpo','erro_ref','ordem','ordem_sujo')
