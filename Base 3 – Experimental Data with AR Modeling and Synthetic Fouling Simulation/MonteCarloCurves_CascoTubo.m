...
% variance_scaling_factor = 0.3; % Worked very well
...
for i = 1:num_realizations
    % Load data
    filename = sprintf('CascoTubo_E%d.mat', i);
    data = load(filename);
    
    % Extract time and temperature
    t_i = data.t;
    T_i = data.Temp_s_casco;
    ...
    k_est = mean(T_interp(end-num_realizations:end));
...
% Shuffle the simulated curves
Sim_Curves = Sim_Curves(:, randperm(num_simulations));
Fouled_Curves = Fouled_Curves(:, randperm(num_fouled));

% Combine all curves
All_Curves = [Exp_Curves, Sim_Curves, Fouled_Curves];

%% Save Data
save('CascoTubo_Data_MC.mat', 'Exp_Curves', 'Sim_Curves', 'Fouled_Curves', 'All_Curves', 'Time_Vector');

disp('Data saved successfully to CascoTubo_Data_MC.mat');

%% Plot Everything Together
figure; hold on;

% Plot the first clean simulated heat exchanger curve (Green, background)
plot(Time_Vector, Sim_Curves(:, 1), 'g', 'LineWidth', 1.5);

% Plot the first fouling heat exchanger curve (Red, middle layer)
plot(Time_Vector, Fouled_Curves(:, 1), 'r', 'LineWidth', 1.5);

% Plot the first experimental curve (Blue, on top)
plot(Time_Vector, Exp_Curves(:, 1), 'b', 'LineWidth', 1.5);

% Add legend for the first curves
legend({'Simulado - Limpo', 'Simulado - Incrusta\c{c}\~ao', 'Experimental'}, 'Location', 'best');

% Plot the rest of the clean simulated heat exchanger curves
for j = 2:num_simulations
    plot(Time_Vector, Sim_Curves(:, j), 'g', 'LineWidth', 1.5, 'HandleVisibility', 'off');
end

% Plot the rest of the fouling heat exchanger curves
for j = 2:num_fouled
    plot(Time_Vector, Fouled_Curves(:, j), 'r', 'LineWidth', 1.5, 'HandleVisibility', 'off');
end

% Plot the rest of the experimental curves
for i = 2:num_realizations
    plot(Time_Vector, Exp_Curves(:, i), 'b', 'LineWidth', 1.5, 'HandleVisibility', 'off');
end
