...
nfonte = 24;
marcador = 15;
num_simulations = 20; % Reduced data availability for target exchanger (20% of source data)
...
% Geometry and initial conditions of TC2
L = 0.5; % Length (m)
D_c = 0.05; % Shell diameter (m)
D_t = 0.025; % Tube diameter (m)
...
T_c_in = 10; % Inlet temperature in the shell (°C)

% Flow rate and fluid properties
m_dot_t = 6e-2; % Mass flow rate in the tube (kg/s)
m_dot_c = 5e-2; % Mass flow rate in the shell (kg/s)
cp_t = 4190; % Specific heat in the tube (J/(kg·K))
cp_c = 4178; % Specific heat in the shell (J/(kg·K))
rho_t = 977.5; % Fluid density in the tube (kg/m^3)
rho_c = 996; % Fluid density in the shell (kg/m^3)

% Thermophysical properties for U calculation
k_t = 0.663; % Thermal conductivity in the tube (W/m·K)
k_c = 0.615; % Thermal conductivity in the shell (W/m·K)
mu_t = 4.04e-4; % Dynamic viscosity in the tube (Pa·s)
mu_c = 7.98e-4; % Dynamic viscosity in the shell (Pa·s)

% Using oil at the tube
cp_t = 2000;
rho_t = 850;
mu_t = 0.1;
k_t = 0.13;
...
passo = 0.05;              % Coarser time step for target exchanger (less accurate instrumentation)
...
for i = 1:num_simulations
    ...
    % Extracting results
    temperatura_tubo_sujo2(:, i) = S(:, 1); % Tube temperature over time
    temperatura_casco_sujo2(:, i) = S(:, 2); % Shell temperature over time
end

%% Saving Results
save dados_AR_TC2.mat t temperatura_casco_limpo temperatura_tubo_limpo temperatura_casco_sujo temperatura_tubo_sujo temperatura_casco_sujo2 temperatura_tubo_sujo2
