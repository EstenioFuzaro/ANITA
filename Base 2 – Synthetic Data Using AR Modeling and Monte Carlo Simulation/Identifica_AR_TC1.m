%--------------------------------------------------------------------------
%   Identify AR Model for a Heat Exchanger - Source
%
% Author: Estênio Fuzaro de Almeida
%--------------------------------------------------------------------------
...
h1 = plot(t, temperatura_casco_limpo(:,1)', 'b'); hold on
h2 = plot(t, temperatura_casco_sujo(:,1)', 'r');
...
plot(t, temperatura_casco_limpo(:,2:end)', 'b', 'HandleVisibility', 'off');
plot(t, temperatura_casco_sujo(:,2:end)', 'r', 'HandleVisibility', 'off');
...
%--------------------------------------------------------------------------
% 100 shell temperature data - clean and fouled (with 25% reduction of
% overall heat transfer coefficient U)

dt = t(2)-t(1); % sampling rate

% clean data - for reference model
np = randperm(100);  % randomly selecting one of the 100 data
...
save akaike_data_he1 na Akaike
%--------------------------------------------------------------------------
% Reference Model
ordem = 6;
...
h1 = plot(X_1(1:99), 'o', 'MarkerSize', 6, 'MarkerFaceColor', 'b'); hold on
h2 = plot(100:199,X_1(100:199), 'o', 'MarkerSize', 6, 'MarkerFaceColor', 'r'); % Add markers with fill
%plot(X_1(200:299), 'o', 'MarkerSize', 6, 'MarkerFaceColor', 'r') % Add markers with fill
min_val = min(X_1(1:99));
...
plot(X_1(1:99), X_2(1:99), 'o', 'MarkerSize', 8, 'MarkerEdgeColor', 'b', 'MarkerFaceColor', 'b') % Light blue filled
hold on
% Plot of points 100 to 199
plot(X_1(100:199), X_2(100:199), 'x', 'MarkerSize', 10, 'LineWidth', 1.5, 'Color', 'r') % Red X
...
indice_limpo = 1:99;
indice_sujo1 = 100:199;
indice_sujo2 = 200:299;

% Visualizar as energias com scatter plots
figure;

% Scatter para curvas limpas (azul)
scatter(indice_limpo, energia_limpo, 50, 'b', 'filled', 'DisplayName', 'Clean');
hold on;

% Scatter para curvas sujas 1 (vermelho)
scatter(indice_sujo1, energia_sujo1, 50, 'r', 'filled', 'DisplayName', 'Fouled');
...
%-------------------------------------------------------------------
save('features_TC1.mat', 'X_1', 'X_2'); % Save to .mat file
save('model_TC1','model_ref','coeficiente_limpo','erro_ref','ordem','ordem_sujo','ordem_sujo2')
