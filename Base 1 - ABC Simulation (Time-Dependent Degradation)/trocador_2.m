...
% 
% Comprimento = 0,5 m
% 	Diâmetro externo do casco = 50 mm
% 	Diâmetro externo do tubo = 25 mm
% 	Espessura das paredes = 2 mm
% 	Diâmetro interno do casco (De - t) = 48 mm
% 	Diâmetro interno do casco (De - t) = 23 mm
% 
% Condições de contorno
% 	Fluxo de massa no inlet do tubo = 7E-2 kg/s
% 	Fluxo de massa no inlet do casco = 5E-2 kg/s
% 	Temperatura do fluido quente (tubo) = 80 °C
% 	Temperatura do fluido frio (casco) = 30 °C
...
YX_casco = casco.Ts_D;
YX_tubo = tubo.Ts_D;
U=180; % Chute inicial do U incrustado
...
%% VARIÁVEIS PARA TRANSFER LEARNING - HE2
month_tube_he2 = [];
temp_tube_he2 = [];
month_shell_he2 = [];
temp_shell_he2 = [];
time_vector_he2 = []; % Novo vetor de tempo para as amostras
...
for i = 1:size(tubo.Ts_v, 1)
    ...
    month_tube_he2 = [month_tube_he2 month_tube];
    temp_tube_he2 = [temp_tube_he2 temp_tube];
    month_shell_he2 = [month_shell_he2 month_shell];
    temp_shell_he2 = [temp_shell_he2 temp_shell];
    
    % Salvar o vetor de tempo amostrado
    time_vector_he2 = [time_vector_he2 r1(indices_colunas(i, :))];
end

% Garantir que o vetor de tempo seja linearizado (caso não esteja)
time_vector_he2 = time_vector_he2(:)';

% Remover valores duplicados em time_vector_he2
[time_vector_he2_unique, unique_indices] = unique(time_vector_he2);

% Atualizar as temperaturas para corresponder aos índices únicos
noisy_temp_tube_he2_unique = temp_tube_he2(unique_indices);
noisy_temp_shell_he2_unique = temp_shell_he2(unique_indices);

% Adicionar ruído às temperaturas (apenas após a remoção de duplicatas)
std_temp_tube_he2 = std(noisy_temp_tube_he2_unique);
noise_tube = randn(size(noisy_temp_tube_he2_unique));
scaled_noise_tube = 0.1 * std_temp_tube_he2 * noise_tube;
noisy_temp_tube_he2 = noisy_temp_tube_he2_unique + scaled_noise_tube;

std_temp_shell_he2 = std(noisy_temp_shell_he2_unique);
noise_shell = randn(size(noisy_temp_shell_he2_unique));
scaled_noise_shell = 0.1 * std_temp_shell_he2 * noise_shell;
noisy_temp_shell_he2 = noisy_temp_shell_he2_unique + scaled_noise_shell;

% Interpolação para Vetor de Tempo Regularizado
% Obter o intervalo mínimo e máximo de tempo no vetor único
time_min = min(time_vector_he2_unique); % Tempo inicial (mínimo global)
time_max = max(time_vector_he2_unique); % Tempo final (máximo global)
...
noisy_temp_tube_he2_regular = interp1(time_vector_he2_unique, noisy_temp_tube_he2, time_vector_regular, 'linear');
noisy_temp_shell_he2_regular = interp1(time_vector_he2_unique, noisy_temp_shell_he2, time_vector_regular, 'linear');

% Salvar dados para 1 ano ou 4 anos, incluindo o vetor de tempo regularizado
if anos == 1
    noisy_temp_shell_he2_1year = noisy_temp_shell_he2_regular;
    noisy_temp_tube_he2_1year = noisy_temp_tube_he2_regular;
    time_vector_1year = time_vector_regular;
    save he2_data_1year noisy_temp_shell_he2_1year noisy_temp_tube_he2_1year time_vector_1year
end
if anos == 4
    noisy_temp_shell_he2_4year = noisy_temp_shell_he2_regular;
    noisy_temp_tube_he2_4year = noisy_temp_tube_he2_regular;
    time_vector_4year = time_vector_regular;
    save he2_data_4year noisy_temp_shell_he2_4year noisy_temp_tube_he2_4year time_vector_4year
end
...
% yticks(0:1:90)
% xticks(0:1:90)
legend('M\^es 0',"M\^es 6",'M\^es 12','FontSize',15)
